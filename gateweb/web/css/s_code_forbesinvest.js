...
var s_account="investopedia-prod"
...
// 10/13/2009 - Timothy Dalby
  if ((document.cookie.indexOf('inv_originalReferrer=') != -1) && (document.cookie.indexOf('inv_originalDomain=') == -1)) {
    wg_extdom = document.cookie.match("inv_originalReferrer=(.*?)(;|$)")[1].match("http\:\/\/(www\\.)?(.*?)(\:.+)?\/");
    if(wg_extdom) wg_extdom = wg_extdom[2];
    document.cookie = 'inv_originalDomain=' + (wg_extdom ? wg_extdom : '' ) + '; domain=.investopedia.com; path=/;';
    //s_vpr('s_prop21',wg_extdom);
    //s_vp_getCookie('s_prop22','wg_originalReferrer');
  }
...
tfs.document.referrer;

// 10/13/2009 - Timothy Dalby
if ("/rotate.aspx"==document.location.pathname &&
document.cookie.indexOf("inv_originalReferrer=")!=-1 && window.wg_extdom && wg_extdom) { r=
document.cookie.match("inv_originalReferrer=(.*?)(;|$)")[1]; }

if(!s_gg("pageURL"))s_wd.s_pageURL=s_fl(l?l:'',
